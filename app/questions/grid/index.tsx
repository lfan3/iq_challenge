...
import React, { useEffect, useRef, useState, MouseEvent, KeyboardEvent, ChangeEvent } from 'react';
...
const GridGame: React.FC<IQuestionContentProps> = (gridProps) => {
  const { selectedQ, data, type, color, explosionTrigger, updateSelectedQ, setIsWrong, questionTxt,correctionArr } = gridProps
  ...
  const [selectedCellId, setselectedCellId] = useState(new Set([]));
  ...
  useEffect(() => {
    ...
    document.addEventListener("mousedown", handleMouseDown as unknown as EventListener)

    return () => {
      document.removeEventListener("mousedown", handleMouseDown as unknown as EventListener)
    }
  }, [])

  useEffect(() => {
    const handleMouseMove = (event: MouseEvent<HTMLDivElement>) => {
      if (isDragging && gridRef.current && gridRef.current.contains(event.target as Node)) {
  ...
  const handleEnterPress = async (event: KeyboardEvent) => {
    ...
    if(!success){
      ...
      if (type == 1) {
        ...
        Object.entries(inputsValue).forEach(([key, value]) => (caseInsensibleInputsValue[key] = value.toLowerCase()))
  
        if (isEqual(caseInsensibleInputsValue, answer)) {
        ...
        const selectedIdArr = Array.from(selectedCellId)
        const compare = isEqual(sortBy(selectedIdArr), sortBy(answer))
        if (compare) {
          explosionTrigger(true)
          const timer = setTimeout(() => {
            updateSelectedQ(selectedQ + 1)
  ...
  const renderContent = (letter: number | string, id: number) => {
    if (letter == "" || letter == "?") {
      return <input className={`w-full h-full grid_answer border-0 outline-0 text-center text-${color}`} value={inputsValue[id] || ""} onChange={(e) => handleInputChange(e, id)} />
  ...
  return (
    ...
    <div className="flex flex-col items-center rounded relative">
      ...
      <div className={`${styles.grid_container} ${styles.grid_15} border-${color}`} ref={gridRef}>
        {data.map((nums, row) => {
          return nums.map((letter, col) => {
            ...
            return (
              <div
                key={id}
                className={`${styles.grid_cell} border-${color} rgb(245 245 245) ${selectedCellId.has(`${row * 100}${col}`) ? `bg-${color} selected-q` : ''}`}
                data-row={row}
                data-col={col}
              >
                {
...
const Grid: React.FC<IQuestionContentProps> = (gridProps) => {
  ...
  return (
    <div className="w-[468px] w-4/12 h-[30rem] flex flex-col items-center rounded relative">
      {/* <p className={`p-1 text-center ${selectedQ > 25 ? `-mt-8 text-slate-500` : '-mt-2 p-3'}`}>{questionTxt}</p> */}
      <GridGame {...gridProps} setIsWrong={setIsWrong} questionTxt={questionTxt}/>
      <div className={`absolute w-full h-full ${type == 2 && "mt-10"  } backdrop-blur-sm backdrop-grayscale ${isWrong ? 'block' : 'hidden'}`}>
        <p className={`absolute w-fit text-6xl top-1/2 -translate-y-1/2  left-1/2 -translate-x-1/2 `}>
          😭
        </p>
      </div>
    </div>
